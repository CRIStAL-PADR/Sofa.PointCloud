...
def createScene(root):
    ...
    Object("InteractiveCamera", name="camera", position=[3, -1, 7, 0, 0, -1, 0], computeZClip=False, zFar=1000)
    
    Object('OglViewport', name='viewportView', 
                cameraPosition= [0, 1, 5],
                cameraOrientation= [ 0, 0, -1, 0],
                screenPosition=[0,0], 
                screenSize=[300, 150],
                useFBO= False,
                zNear= 0.1,
    ...
    if useOcTree:
        Object("PointCloudOctreeRenderer", name="ocTreeRenderer", camera=root.camera.linkpath) 
    else:
    ...
    with Node("Modelling"):
        if useOcTree:
            with OcTree("ocTree", position=[0,0,0,0,0,0,1], splats=["splats/truck.ply"], distances=[1000]) as octree:
                octree.addObject(DofManipulator(name="manipulator", direction=1, target=octree))
        else:
            with Container("container", position=[0,0,0,0,0,0,1], splat="splats/truck.ply") as conatainer:
